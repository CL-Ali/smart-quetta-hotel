 ...
 * Currently a no-op — wire this up to your preferred channel
 * (email, WhatsApp, Slack, Firebase FCM, etc.).
 */
export async function notifyOwner(payload: NotificationPayload): Promise<boolean> {
  const { title, content } = validatePayload(payload);

  // TODO: implement notification channel, e.g.:
  //   await sendEmail({ to: OWNER_EMAIL, subject: title, body: content });
